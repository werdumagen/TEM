...
class EditorState:
    ...
    def _show_tooltip_for_idx(self, idx):
        ...
        y, x = self.points[idx]; inten = None; is_percentile_like = False
        if hasattr(self, '_percent_map') and self._percent_map is not None: H, W = self._percent_map.shape[:2]; yi = max(0, min(H - 1, int(round(y)))); xi = max(0, min(W - 1, int(round(x)))); inten = float(self._percent_map[yi, xi]); is_percentile_like = True
        elif hasattr(self, 'values') and self.values is not None and idx < len(self.values): inten = float(self.values[idx]); is_percentile_like = 0 <= inten <= 100
        txt = f"x={x:.1f}, y={y:.1f}\n";
        if inten is not None: unit = "%" if is_percentile_like else "raw"; txt += f"I={inten:.1f} {unit}"
        else: txt += "Intensity: N/A"
        self._clear_tooltip()
        self._tooltip = self.ax.annotate(txt, xy=(x, y), xytext=(10, 10), textcoords="offset points", bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="black", alpha=0.9), fontsize=9, zorder=20)
        self._tooltip_idx = idx; self.canvas.draw_idle()
