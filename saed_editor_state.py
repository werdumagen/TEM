...
class EditorState:
    ...
    # ... (Методы Tooltip и Measurement без изменений) ...
    def _remove_measure_preview_artist(self) -> bool:
        if hasattr(self, '_measure_preview_artist') and self._measure_preview_artist is not None:
            try:
                self._measure_preview_artist.remove(); self._measure_preview_artist = None; return True
            except Exception:
                pass; self._measure_preview_artist = None
        return False
    ...
    def _remove_measurement_artists(self) -> bool:
        ...
        if hasattr(self, '_measure_line_artist') and self._measure_line_artist is not None:
            try:
                self._measure_line_artist.remove(); removed = True; self._measure_line_artist = None
            except Exception:
                pass; self._measure_line_artist = None
        if hasattr(self, '_measure_annotation') and self._measure_annotation is not None:
            try:
                self._measure_annotation.remove(); removed = True; self._measure_annotation = None
            except Exception:
                pass; self._measure_annotation = None
        return removed
    ...
    def _start_measurement_from_center(self) -> bool:
        ...
        self._measure_start_idx = CENTER_AS_POINT_IDX;
    ...
    def _update_measurement_preview(self, pos: tuple[float, float] | None) -> None:
        ...
        if not hasattr(self, '_measure_preview_artist') or self._measure_preview_artist is None:
            (line,) = self.ax.plot([x0, x1], [y0, y1], color="#ffcc33", lw=1.6, ls="--", alpha=0.9, scalex=False,
    ...
    def _clear_tooltip(self, *, keep_measure: bool = False, keep_preview: bool = False):
        ...
        if hasattr(self, '_tooltip') and self._tooltip is not None:
            try:
                self._tooltip.remove(); self._tooltip = None; self._tooltip_idx = None; removed_tooltip = True
            except Exception:
                pass; self._tooltip = None; self._tooltip_idx = None
    ...
    def _show_tooltip_for_idx(self, idx):
        ...
        if hasattr(self, '_percent_map') and self._percent_map is not None:
            H, W = self._percent_map.shape[:2]; yi = max(0, min(H - 1, int(round(y)))); xi = max(0, min(W - 1,
                                                                                                        int(round(
                                                                                                            x)))); inten = float(
                self._percent_map[yi, xi]); is_percentile_like = True
        elif hasattr(self, 'values') and self.values is not None and idx < len(self.values):
            inten = float(self.values[idx]); is_percentile_like = 0 <= inten <= 100
        txt = f"x={x:.1f}, y={y:.1f}\n";
        if inten is not None:
            unit = "%" if is_percentile_like else "raw"; txt += f"I={inten:.1f} {unit}"
        else:
            txt += "Intensity: N/A"
        self._clear_tooltip()
        self._tooltip = self.ax.annotate(txt, xy=(x, y), xytext=(10, 10), textcoords="offset points",
    ...
    def _apply_snapshot(self, snap):
        ...
        # --- НАЧАЛО ИСПРАВЛЕНИЯ (бывшая строка 161) ---
        # Эта строка вызывала SyntaxError:
        # if hasattr(self, 'rect_artist') and self.rect_artist is not None: try: self.rect_artist.remove() except Exception: pass; self.rect_artist = None
        ...
        # Восстанавливаем точки, значения, типы
        self.points = snap["points"].copy()
        self.values = snap["values"].copy()
        # --- ИЗМЕНЕНО: Восстанавливаем типы ---
        self.point_types = list(snap.get("point_types", []))  # Восстанавливаем как список
