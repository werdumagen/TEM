...
from typing import Tuple, Dict, Any, Optional  # 22
# 23
...
def refine_center_antipodal(center: Tuple[float, float], pts: np.ndarray, tol_ang_deg: float = 8.0,
                            tol_rel_r: float = 0.06, iters: int = 3) -> CenterResult:  # 251
...
class SAEDLauncherFrame(ttk.Frame):  # 277
    ...
    def _build_ui(self):  # 289
        ...
        ttk.Button(data_box, text="Browse…", command=self._browse_img).grid(row=0, column=3, sticky="ew", padx=6,
                                                                            pady=4)  # 306
        # 307
        ttk.Label(data_box, text="Project Folder:").grid(row=1, column=0, sticky="w", padx=6, pady=4)  # 308
        self.ent_project_folder = ttk.Entry(data_box)  # 309
        self.ent_project_folder.insert(0, self._get_default_output_path())  # 310
        self.ent_project_folder.grid(row=1, column=1, columnspan=2, sticky="we", padx=6, pady=4)  # 311
        ttk.Button(data_box, text="Choose…", command=self._browse_project_folder).grid(row=1, column=3, sticky="ew",
                                                                                       padx=6, pady=4)  # 312
        # 313
    ...
    # 536
    def _browse_project_folder(self):  # 537
        p = filedialog.askdirectory(title="Select Project Folder")  # 538
        if p: self.ent_project_folder.delete(0, tk.END); self.ent_project_folder.insert(0, p)  # 539

    # 540
    # --- NEW: Session Management Methods ---
    def get_project_folder(self) -> Optional[Path]:
        p_str = self.ent_project_folder.get()
        if not p_str:
            return None
        return Path(p_str)

    def set_project_folder(self, folder_path: Path):
        self.ent_project_folder.delete(0, tk.END)
        self.ent_project_folder.insert(0, str(folder_path))

    def get_state(self) -> Dict[str, Any]:
        """Collect all launcher settings into a dictionary."""
        return {
            ...
            "perc": self.spn_perc.get(),
            "merge_perc": self.spn_merge_perc.get(),
            "merge_rad": self.spn_merge_rad.get(),
            "merge_tol": self.spn_merge_tol.get(),
            "minsep": self.spn_minsep.get(),
            "maxpts": self.spn_maxpts.get(),
            "iters": self.spn_iters.get(),
            "tolang": self.spn_tolang.get(),
            "tolr": self.spn_tolr.get(),
            "dead": self.spn_dead.get(),
            "search": self.spn_search.get(),
        }

    def set_state(self, state: Optional[Dict[str, Any]], project_folder: Path):
        """Apply settings from a loaded state dictionary."""
        self.set_project_folder(project_folder)
        if state is None:
            return

        self.ent_img.delete(0, tk.END)
        self.ent_img.insert(0, state.get("image_path_str", ""))
        self.ent_cx.delete(0, tk.END)
        self.ent_cx.insert(0, state.get("center_x", ""))
        self.ent_cy.delete(0, tk.END)
        self.ent_cy.insert(0, state.get("center_y", ""))

        self.cmb_pre.set(state.get("preproc_mode", "No smoothing"))
        self._on_preproc_change(None)  # Apply UI changes
        ...
        self._set_spinbox_value(self.spn_perc, state.get("perc", 99.0))
        self._set_spinbox_value(self.spn_merge_perc, state.get("merge_perc", 95.0))
        self._set_spinbox_value(self.spn_merge_rad, state.get("merge_rad", 0))
        self._set_spinbox_value(self.spn_merge_tol, state.get("merge_tol", 10.0))
        self._set_spinbox_value(self.spn_minsep, state.get("minsep", 5))
        self._set_spinbox_value(self.spn_maxpts, state.get("maxpts", 6000))
        self._set_spinbox_value(self.spn_iters, state.get("iters", 4))
        self._set_spinbox_value(self.spn_tolang, state.get("tolang", 8.0))
        self._set_spinbox_value(self.spn_tolr, state.get("tolr", 0.06))
        self._set_spinbox_value(self.spn_dead, state.get("dead", 0))
        self._set_spinbox_value(self.spn_search, state.get("search", 0))
    ...
    def _go_editor(self):  # 541
        try:  # 542
            ...
            if not image_path_str:
                ...
                return
            image_path = Path(image_path_str).expanduser()  # 543
            if not image_path.exists():  # 544
                messagebox.showerror("Error", "Image not found");
                return  # 545

            project_dir = self.get_project_folder()
            if not project_dir:
                messagebox.showerror("Error", "Project Folder not set");
                return
            project_dir.mkdir(parents=True, exist_ok=True)  # 546
            # 547
            ...
            cy_txt = self.ent_cy.get().strip()  # 576
            ...
            if len(pts) < 120:  # 585
                pts = detect_spots(arr, perc=max(97.5, perc - 1.0), win=loc_win, min_sep=min_sep,
                                   max_spots=max_pts)  # 586
            # 587
            ...
            # Use project_dir as the base for all outputs
            # Use image name to create a unique-ish name, but in the project folder
            base_output_name = image_path.stem
            ...
            saed_input_path = project_dir / f"{base_output_name}_saed_input.json"  # 623
            ...
            (project_dir / f"{base_output_name}_center_init.json").write_text(json.dumps({  # 642
