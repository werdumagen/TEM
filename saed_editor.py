...
class PointEditor(tk.Frame):
    ...
    def __init__(self, master: tk.Misc, controller=None,
        ...
        if auto_load and input_json:
            try:
                # load_input_json вызовет _calculate_initial_symmetry и _update_group_panel
    ...
    def _build_ui(self):
        ...
        group_panel_host.columnconfigure(0, weight=1)

        group_box = ttk.LabelFrame(group_panel_host, text="Group Inspector", padding=(12, 10, 12, 12))
        group_box.grid(row=0, column=0, sticky="nsew")
        ...
        self.group_tree.column("count", width=60, anchor="e")

        tree_scroll = ttk.Scrollbar(group_box, orient="vertical", command=self.group_tree.yview)
    ...
    def _on_group_select(self, event=None):
        """Вызывается при клике на элемент в Group Inspector."""
        if not self.group_tree:
            return

        try:
            selected_iids = self.group_tree.selection()
            if not selected_iids:
                # Снимаем выделение, если кликнули мимо
                if self.ui_state.ring_select_indices:
                    self.ui_state.ring_select_indices.clear()
                    self.set_status("Group selection cleared.")
                    self.redraw()
                return
            ...
            if selected_iid in self.group_tree_map:
                ...
                # Используем ui_state.ring_select_indices для хранения выделения
