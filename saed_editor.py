...
from saed_editor_handlers_refactored import EditorEventHandlers # Функции симметрии/группировки удалены оттуда
...
class PointEditor(tk.Frame):
    ...
    def __init__(self, master: tk.Misc, controller=None,
        ...
        # УДАЛЕНО: UI для Group Inspector
        ...
        # УДАЛЕНО: Подключение btn_auto_group
        # УДАЛЕНО: Подключение cmb_symmetry
        # УДАЛЕНО: Подключение group_tree
        ...
        if auto_load and input_json:
            try:
                # Загрузка JSON больше не вызывает _calculate_initial_symmetry или _update_group_panel
    ...
    # ---------- UI Builder (Упрощенный) ----------
    def _build_ui(self):
        """Создает Tkinter виджеты и Matplotlib холст (без панелей симметрии/групп)."""
        self.columnconfigure(1, weight=1) # Canvas (теперь занимает все оставшееся место)
        # УДАЛЕНО: columnconfigure для Group Panel
        ...
        # УДАЛЕНО: Auto-Grouping Frame
        # УДАЛЕНО: Symmetry Panel Frame

        # --- Display Options ---
        # ttk.Separator(controls, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=(12, 10)) # Можно убрать разделитель, если он был последним
        ...
        # --- Help Panel (с упрощенным текстом) ---
        ...
        help_text = (
            ...
            # УДАЛЕНЫ подсказки про Auto Ring Grouping, Symmetry Panel, Group Inspector
        )
        ...
        # --- Центральная панель (холст Matplotlib) ---
        canvas_frame = ttk.Frame(self, padding=(0, 16, 16, 16))
        canvas_frame.grid(row=0, column=1, sticky="nsew") # Занимает колонку 1
        ...
        # УДАЛЕНО: Правая панель (Group Inspector)


    # ---------- Управление Контроллером (Упрощено) ----------

    def redraw(self):
        """Публичный метод для запуска перерисовки."""
        if self.view:
    ...
    # ---------- Обработчики UI Контроллера (Упрощено) ----------
    ...
    # УДАЛЕНО: _on_symmetry_change
    # УДАЛЕНО: _update_group_panel
    # УДАЛЕНО: _on_group_select

    # ---------- Управление состоянием (Undo/Redo) (Упрощено) ----------
    def push_undo(self):
    ...
    def get_center(self) -> Optional[Tuple[float, float]]:
        if self.overlay and isinstance(self.overlay.get("center"), dict):
            ...
            if "x" in c and "y" in c: return (float(c["y"]), float(c["x"]))
        return None
...
class PointEditorApp(tk.Tk):
    def __init__(self, input_json: str | None = None):
        super().__init__()
        self.title("SAED Editor (Simplified)") # Изменено название
        self.geometry("1100x800") # Уменьшена ширина
