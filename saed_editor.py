...
class PointEditor(tk.Frame):
    ...
    def _build_ui(self):
        ...
        # ... (Код для scroll_host, canvas, vscroll, scrollable_frame) ...
    ...
    def push_undo(self):
        """Сохраняет слепок Модели в стек Undo."""
    ...
    def push_redo(self):
        """Сохраняет слепок Модели в стек Redo."""
        if self._history_cap > 0:
            self.redo.append(self.model.get_snapshot())
            if len(self.redo) > self._history_cap: self.redo.pop(0)

    def pop_undo(self):
        """Удаляет последнее состояние Undo (используется при отмене действия)."""
        if self.undo:
            self.undo.pop()

    def _apply_snapshot(self, snap: Dict[str, Any]):
        """
        Применяет слепок к Модели.
        Примечание: Сброс состояния UI (замеры, выделения)
        происходит в _undo_btn/_redo_btn.
        """
    ...
    def _undo_btn(self, event=None):
        ...
        self.ui_state.cancel_all_interactions()  # Сбрасываем UI
        self.ui_state.clear_tooltip(self.view)

        self.push_redo()  # Сохраняем текущее состояние в Redo
    ...
    def _redo_btn(self, event=None):
        ...
        self.push_undo()  # Сохраняем текущее состояние в Undo
    ...
    def get_image_to_display(self) -> Optional[np.ndarray]:
        """Возвращает массив изображения для отрисовки (Raw или Processed)."""
    ...
    def ensure_view_center(self):
        """Делегирует проверку центра во View."""
        if self.view:
    ...
    def get_center(self) -> Optional[Tuple[float, float]]:
        """Возвращает (y, x) центра, если он есть."""
    ...
    def sample_intensities(self, pts_yx: np.ndarray) -> np.ndarray:
        """Сэмплирует значения интенсивности (используется IO и Handlers)."""
...
class PointEditorApp(tk.Tk):
    def __init__(self, input_json: str | None = None):
        super().__init__()
        self.title("SAED Editor + Analysis (Refactored)")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Make sure all .py files (preproc.py, percentile_utils.py) are in the same directory.")
        sys.exit(1)
