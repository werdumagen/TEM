...
# User input
try:
    divisions = int(input("Enter the desired number of tiling layers/subdivisions (ex. '7'): "))
except ValueError:
    print("\nInvalid input: Only integers above 0 work")
    raise SystemExit(0)

try:
    zoom = input("Would you like the image zoomed in our out (ex. 'in'): ")
except ValueError:
...
try:
    r1, r2 = [int(r) for r in
              tuple(input("Enter the desired image resolution, separated by a space (ex: '1080 1080'): ").split())]
except ValueError:
    print("\nInvalid input: the format should be: ['integer' 'integer']")
    raise SystemExit(0)

try:
    c1, c2, c3 = tuple(input("Enter the desired colors, separated by spaces (ex: 'red blue grey'): ").split())
except ValueError:
...
for c in c1, c2, c3:
    try:
        colors.append({
                          "random": [random.randint(0, 256) / 256, random.randint(0, 256) / 256,
                                     random.randint(0, 256) / 256],
                          "red": [0.8, 0.3, 0.3],
                          "orange": [0.9, 0.6, 0.3],
                          "yellow": [0.6, 0.9, 0.3],
                          "green": [0.3, 0.9, 0.6],
