...
Взаимодействует с Моделью (model) для заполнения/чтения данных.
"""
import json
...
# Импорты из проекта
# (Предполагается, что preproc.py и percentile_utils.py находятся рядом)
try:
    ...
    messagebox.showerror("Import Error",
                         f"Failed to import dependencies: {e}\nEnsure preproc.py and percentile_utils.py are present.")


    # Определяем заглушки, чтобы приложение не упало
...
class EditorIO:
    ...
    # ---------- Обертки для кнопок UI ----------

    def open_json_wrapper(self):
        """Обертка для кнопки 'Open JSON'."""
    ...
    def load_input_json(self, path: Path, *, push_undo: bool = False, reset_view: bool = True):
        ...
        # --- 4. Загрузка Overlay (Центр/Радиусы) ---
        ...
        # --- 5. Загрузка Точек (через Модель) ---
        try:
            pts_data = data.get("points", [])
            # Передаем Модели список точек, функцию для сэмплинга и центр
            self.controller.model.from_json_list(
                pts_data,
                self.controller.sample_intensities,
                center_yx
            )
        ...
        if reset_view:
            self.controller.view_cx = None
            self.controller.view_cy = None
        self.controller.ensure_view_center()  # Установит центр, если view_cx is None
    ...
    def _save_points(self) -> Path:
        ...
        # --- 1. Получаем список точек из Модели ---
        ...
        # --- 3. Сохраняем saed_input.edited.json ---
    ...
    def set_state(self, state: dict):
        ...
        if not image_path_str:
            self.controller.clear_all()  # Метод в контроллере для сброса
            return

        try:
            self.controller.image_path = Path(image_path_str).resolve()
            if not self.controller.image_path.exists():
                raise FileNotFoundError(f"Image not found: {self.controller.image_path}")

            # 1. Восстанавливаем Контроллер
            self.controller._preproc_settings = PreprocSettings.from_json(state.get("preproc_settings", {}))
            self.controller.img_arr_raw = load_grayscale_with_preproc(self.controller.image_path,
                                                                      PreprocSettings(mode="raw"))
            self.controller.img_arr_processed = load_grayscale_with_preproc(self.controller.image_path,
                                                                            self.controller._preproc_settings)
            p_map, uniq_vals, uniq_perc = compute_percentile_map(self.controller.img_arr_processed)
            self.controller._percent_map = p_map
            self.controller._percent_lookup = (uniq_vals, uniq_perc)
            ...
            self.controller.view_cx = state.get("view_cx")
            self.controller.view_cy = state.get("view_cy")
            ...
            self.controller.ui_state.cancel_all_interactions()  # Сбрасываем artists
            self.controller.ui_state.center_dragging = False
            self.controller.ui_state.rect_start = None
    ...
    def save_debug_data(self, filename: str = "auto_grouped_points_debug.json"):
        """Сохраняет отладочные данные из Модели."""
    ...
    def start_analysis_wrapper(self):
        ...
        try:
            ...
            payload = {
                ...
                "centers": {
                    ...
                    "overlay": self.controller.overlay.get("center")
                },
                "radii": {
                    "dead": self.controller.overlay.get("dead_radius", 0.0),
                    "search": self.controller.overlay.get("search_radius", 0.0)
        ...
        # Передаем управление в pipeline_app
        if self.controller.app_controller is not None:
            try:
                self.controller.app_controller.open_analysis(
